...
The causal networks enable 1) visual representation of the complex inter-relationships between scenario inputs and outcomes across a range of objectives and 2) assessment of outcomes aggregated along the thematic dimension (see @sec-aggregator). The former aids transparency, elucidating the intentions and causal relationships behind the response models and is a useful device for communication alongside other final outputs. The latter allows outcomes to be quantified for individual (or sets of) values, including environmental objectives, target groups, long-term targets, or at the overarching level of environmental, cultural, social, or economic values. This quantification provides a powerful assessment tool and the ability to identify synergies and trade-offs across interrelated values.
...
# Huxtables look better, but won't go side-by-side
...
obj_in_groups <- obj_sdl_to_plot |>
  dplyr::filter(scenario %in% c(scenarios_to_plot, "MAX"))  |> 
  filter(env_group != 'EB') |> # There's only one, this is just distracting.
  filter(scenario %in% c(scenarios_to_plot, 'MAX')) |> 
  # clean the names
  mutate(scenario = ifelse(scenario == 'MAX', 'MAX', 
                           paste0(climate_code, adapt_code)), 
         SWSDLName = stringr::str_replace(SWSDLName, '–', '-\n')) |> 
  plot_outcomes(y_col = 'ewr_achieved',
                y_lab = "EWR achievement\n (proportion of objective)",
...
obj_in_groups <- obj_in_groups + 
  ggplot2::geom_col(data = filter(obj_in_groups$data, scenario %in% c("MAX")), 
...
gl_difference <- obj_sdl_to_plot |>
  ...
  plot_outcomes(y_col = 'ewr_achieved',
                ...
                colorgroups = 'env_group',
                colorset = 'env_obj',
                # pal_list = list('ggthemes::Tableau_20'),
                pal_list = env_pals,
                # base_lev = 'E1',
                # comp_fun = 'difference',
...
sdl_smooth_groups <- obj_sdl_to_plot |>
  ...
  plot_outcomes(y_col = 'ewr_achieved',
                ...
                y_lab = 'Environmental objectives',
...
*I'll make this plot work better. We need a way to do legends on the plots. And panels. In extremis, I can save this out and read it back in as an image, but I'd like to actually fix the plot code. Leaving it ugly for now.*
...
![Aggregation functions determine outcomes and reflect processes or values. Aggregating a set of outcomes (top row) yields a different outcome depending on the function used. Thus, function choice should be considered carefully and reflect the processes involved or management goals. The toolkit provides default functions for the mean, compensating (e.g. max), limiting (min), and spatially-weighted versions of the same. It also provides the ability for the user to define any desired function, allowing for more complex situations.](../images/agg_concept.png){#fig-aggregation_types alt="Aggregation functions determine outcomes and reflect processes or values. Aggregating a set of outcomes (top row) yields a different outcome depending on the function used. Thus, function choice should be considered carefully and reflect the processes involved or management goals. The toolkit provides default functions for the mean, compensating (e.g. max), limiting (min), and spatially-weighted versions of the same. It also provides the ability for the user to define any desired function, allowing for more complex situtations."}
...
The goal here is to show the values at the sdl scale, but also to show how the gauges aggregate to that scale. See which one works best, and make suggestions for improvements, please. In the text, @fig-gauge-to-sdl-map has just a large number of env_objs and scenarios in a and the gauges on grey sdl units in b. @fig-gauge-to-sdl-map-few is similar, it just reduces the panels in a so it's easier to see which matches b. @fig-gauge-to-sdl-map-stepwise-agg shows both steps of the gauge to sdl aggregation in panel b. @fig-gauge-to-sdl-map-gaugecolor has the gauges and sdl colors overlaid in b, but that requires using different color ramps for the same variable, which is confusing. @fig-gauge-to-sdl-map-gaugecolor-same does the same overlay, but uses the same colorramp, which is better for meaning, but worse to see what's happening (maybe?).

```{r}
#| label: fig-gauge-to-sdl-map-few
#| fig-cap: "" 

# reduced set of objectives and scenarios- closer to the next option
(ef_maps_few + theme(legend.position = 'none') ) + 
  wrap_elements(ef_gauges + 
                  guides(color = guide_colourbar(title.position="top")) +
                  theme(legend.position = 'bottom')) + 
  plot_annotation( tag_levels = 'a') +
...
The causal networks shown in the text have been subset for readability, and baselined. Here, we have the
