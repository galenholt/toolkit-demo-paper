...
NEED TO DO - GEORGIA
OTHER notes from georgia: “aggregation along causal network” OR "aggregation along causal dimension”
ONLY three catchements were used originally because those were the only ones that we did the causal network for. now all of NSW (maybe basin has networks) should we expand?
...
![Conceptualisation of toolkit archetecture (blue arrows) and benefits to management (yellow arrows). Scenarios, represented by hydrographs, reflect historic flows or modeled scenarios (produced via hydrological modelling outside the toolkit). These hydrographs are fed in a consistent way to various response models (see @sec-modules) via the Flowre Controller (see @sec-controller). Additional inputs of spatial data (see @sec-spatial_data) and causal networks (see @sec-causal_networks) provide grouping information for scalling via the Flowre Aggregator (see @sec-aggregator). The results are synthesized into management-relevant outputs that aid decision-making via the Flowre Comparer (see @sec-comparer). *GALEN/GEORGIA NEED MORE ABOUT THE YELLOW PATHWAY* ](../images/Conceptual_fig_demopaper-01.png){#fig-tktomgmt}
...
TEXT FOR SOMEWHERE ? Spatial units are typically more general, typically polygons of management interest, and require only light changes to make compatible.
...
TEXT FOR SOMEWHERE ? Causal networks are linked closely to each Response model, and while both are defined externally to the toolkit they require significant work to integrate into a compatible, modular toolkit component.
...
TEXT FOR SOMEWHERE?  hydrographs and direct Response model output can directly utilise the Comparer functionality without aggregation. Each component of the Flowre toolkit is distict, allowing modular changes to be made without altering the function of other components.
...
relevant_basin_rivers <- filter(basin_rivers, ConCatID %in% relevant_basin_rivers2$ConCatID)
...
wrap_elements(full = sdl_achieve_bars) + wrap_elements(full = sdl_achieve_map) + 
  patchwork::plot_layout(ncol = 2, widths = c(2,2), heights = c(5,6)) & 
...
#   plot_annotation( tag_levels = 'a')
```
*EF: It's not immediately obvious what condition relative to E1 means, especially when a and b axes are different but labeled the same. for fig 13*
...
*EF: it isn't clear off the bat what condition means here. I assume this is change in environmental objectives relative to the baseline scenario? are the units here for condition, proportional changes from the baseline? refering to the heatmap fig 14*
...
TEXT TO PUT SOMEWHERE - Aggregation functions determine outcomes and reflect processes or values. Aggregating a set of outcomes yields a different outcome depending on the function used. Thus, function choice should be considered carefully and reflect the processes involved or management goals. The toolkit provides default functions for the mean, compensating (e.g. max), limiting (min), and spatially-weighted versions of the same. It also provides the ability for the user to define any desired function, allowing for more complex situtations.
...
Development of scenaros - GEORGIA WRITE SOME WORD HERE
